...
public class SwerveModule {
 ...
 // private final SimpleMotorFeedforward m_driveFeedforward = new SimpleMotorFeedforward(1, 3);
 // private final SimpleMotorFeedforward m_turnFeedforward = new SimpleMotorFeedforward(.5, 0.25);
   ...
   * @param turningEncoderA
   * @param turningEncoderB
   */
  public SwerveModule(int driveMotorChannel, int turningMotorChannel, int turningEncoderA, int turningEncoderB) {
    ...
    m_driveEncoder.setPositionConversionFactor(2 * Math.PI * kWheelRadius / 42);
    // Set the distance (in this case, angle) per pulse for the turning encoder.
    // This is the the angle through an entire rotation (2 * wpi::math::pi)
    // divided by the encoder resolution.
    m_turningEncoder.setDistancePerPulse(2 * Math.PI / 4096);
